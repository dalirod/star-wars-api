...
# devuelve lista de x serializados pero en diccionario
...
def get_planet_id(planet_id=None):
    if request.method == "GET":
    ...
    if request.method == "POST":
        ...
        if name is None or gravity is None or population is None or climate is None:
            ...
            try:
                ...
                db.session.commit()
                return jsonify({"message": "planetas creados"}),200
            except Exception as error:
                return jsonify(error.args[0]),error.args[1]
...
@app.route('/favorite/planet/<int:nature_id>', methods = ['POST'])  
def getplanet_id_favorite(nature_id=None):
...
@app.route('/favorite/people/<int:nature_id>', methods = ['POST'])
def getpeople_id_favorite(nature_id=None):
...
def planet_id_delete(nature_id=None):
...
@app.route('/favorite/people/<int:nature_id>', methods = ['DELETE'])       
def people_id_delete(nature_id=None):
